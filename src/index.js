...
app.get('/tasks',async(req,res)=>{
    ...
    // Task.find({}).then((task)=>{res.send(task)}).catch((e)=>{res.status(500).send()})
...
//     Task.findById(_id).then((task)=>{if(!task){return res.status(404).send()
//     console.log(task)
//     }
//     res.send(task)
// }).catch((e)=>{res.status(500).send()})

// console.log('Req. Task ID:' + _id)
})
